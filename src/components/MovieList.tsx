...
const MovieCard: React.FC<{ movie: Movie; index: number; displayIndex: number }> = ({
  movie,
  displayIndex,
}) => (
  <div
  ...
  >
    ...
    <div className="flex flex-col md:flex-row md:pl-[8%] w-full md:space-x-4">
      <div className="py-3 flex flex-col text-white justify-center items-center md:w-[100%]">
        <h3 className="text-xl text-[#FFD700] text-center font-BebasNune">{movie?.title}</h3>
...
const MovieList: React.FC = () => {
  ...
  const totalPages = useMemo(() => Math.ceil((movies?.length || 0) / moviesPerPage), [movies]);
